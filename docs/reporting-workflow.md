# 보고서 작성 및 이력 관리 워크플로우

이 문서는 GitHub, Linear, Notion을 함께 사용할 때 어떤 정보를 어디에 남길지 정리한 운영 기준입니다.
목표는 교수님이 요구한 “어떤 문제를 참고했고, 어떤 제약 아래에서 무엇을 개선했는지”를 결과 보고서에 쉽게 옮기는 것입니다.

운영 원본은 GitHub입니다.
Linear와 Notion은 GitHub 이슈/PR을 더 보기 좋게 정리하는 보조 수단이며, GitHub에 없는 정보는 공식 개발 이력으로 간주하지 않습니다.

## 0. 기본 운영 결론

1. 새 문서를 계속 만들지 않습니다. 공식 운영 문서는 `docs/constraints-and-decisions.md`, `docs/engineering-history.md`, `docs/reporting-workflow.md` 3개를 기준으로 합니다.
2. 문제는 GitHub Issue로 시작하고, 코드 변경은 PR로 검증하며, 보고서 문장은 docs에 요약합니다.
3. Linear와 Notion은 보조 인덱스입니다. GitHub 링크가 없는 Linear/Notion 항목은 공식 개발 이력으로 쓰지 않습니다.
4. 개선 중에는 간단 기록만 남겨도 됩니다. 이후 `git log`, PR 본문, 테스트 로그를 보고 상세 과정을 채웁니다.
5. 제약은 `C1~C8` 코드로 분류합니다. 제약 코드가 없는 기록은 “왜 어려웠는지” 설명하기 어렵기 때문에 보고서 근거로 약합니다.

## 1. 도구별 역할

| 도구 | 역할 | 남길 내용 |
| --- | --- | --- |
| GitHub PR | 코드 변경의 공식 이력 | 변경 파일, 구현 이유, 검증 명령, merge commit |
| GitHub Issue | 문제와 증거의 공식 시작점 | 오탐/미탐/성능/UI/문서 유형, URL, 캡처, 기대/실제 결과 |
| Linear | 반복 개선과 문제 추적 | 실제 문제, 캡처/URL, 가설, 실험 결과, 남은 이슈 |
| Notion | 보고서용 정리본 | 주차별 요약, 제약 사항, 개선 전후 비교, 발표 문장 |
| `docs/engineering-history.md` | repo 내부 근거 문서 | 핵심 개선 흐름, PR별 의미, 보고서용 문장 |
| `docs/constraints-and-decisions.md` | 제약/의사결정 문서 | 왜 특정 방식을 선택하거나 보류했는지 |

## 2. Notion Evidence Hub 구조

Notion `Bugs & Issues`는 커밋/작업 로그 보드가 아니라 심사 대응 evidence hub로 사용합니다.
상위 DB에는 토픽 Hub 카드만 유지하고, 실제 사례/실험/metric/screenshot/trace는 각 Hub 내부 child DB row에 누적합니다.
커밋 하나당 상위 카드를 만들지 않습니다.

상위 DB 공통 속성은 아래 기준을 유지합니다.

| 속성 | 타입 | 예시 |
| --- | --- | --- |
| `Issue Title` | Title | `Android boundsInScreen Masking Hub` |
| `Project` | Text | `Chungmaru` |
| `Sprint` | Text | `Evidence Ledger` |
| `Status` | Status | Backlog, In progress, In Review, Done, Blocked |
| `Type` | Select | Feature, Improvement, Bug, Tech Debt |
| `Impact Area` | Multi-select | UI, Backend, API, Frontend, Database |
| `Description` | Text | Hub 목적 1문장 |
| `Link / Attachment` | URL | 관련 GitHub PR 또는 artifact |

상위 Hub는 아래 10개로 고정합니다.

| Hub | 실제 row를 쌓는 child DB |
| --- | --- |
| `Model Evaluation Hub` | `Model Evaluation Runs`, `Model Comparison Summary` |
| `Model Error Cases Hub` | `Model Error Cases`, `Score Bucket Analysis` |
| `Clean-topic Bias Hub` | `Clean Topic Tests` |
| `Span & Masking Trace Hub` | `Masking Trace Examples` |
| `Parser / Extractor Hub` | `Raw Cleaned Snapshots`, `UI Removal Rules` |
| `Platform Parser Issues Hub` | `Platform Parser Issues` |
| `Android boundsInScreen Masking Hub` | `Android Bounds Masking Packs` |
| `Chrome / Android E2E Demo Hub` | `E2E Demo Traces` |
| `Demo Capability Matrix Hub` | `Demo Capability Matrix` |
| `Evidence / Git / Worktree Policy Hub` | `Evidence Decisions`, `Worktree / Branch Records` |

권장 뷰는 Hub별 child DB에서 만듭니다.

1. `Report-ready`: `Report Include? = Yes`와 `Status = Verified/Report-ready`만 봅니다.
2. `Open Gaps`: status가 Draft/Backlog/In progress인 row만 봅니다.
3. `By Platform`: Android, Chrome, YouTube, Instagram, TikTok surface별로 묶습니다.
4. `By Failure Type`: FP/FN, OCR-needed, bounds-wrong, UI-noise 같은 실패 유형별로 묶습니다.

기존 상세 카드는 삭제하지 않고 관련 Hub 아래로 이동해 흡수합니다.
상위 DB에 남기는 것은 Hub뿐이고, 실제 증거는 child DB row로만 추가합니다.

## 3. Linear 이슈/코멘트 템플릿

```md
## YYYY-MM-DD 개선 루프

### 문제
- 실제 사용자 화면 또는 콘솔에서 관찰된 문제:

### 증거
- URL:
- 캡처:
- 로그:

### 제약
- 유지해야 하는 계약:
- 건드리지 않아야 하는 영역:

### 가설
- 원인 후보:

### 수정
- 변경 파일:
- 선택한 방식:
- 대안과 보류 이유:
- 사용 기술:

### 검증
- 자동 검증:
- 수동 검증:

### 결과
- 해결된 것:
- 남은 것:
- 보고서 반영 여부:
```

## 4. GitHub PR 본문 템플릿

```md
## 요약
- 무엇을 개선했는지 2~3줄로 작성

## 문제와 근거
- 어떤 화면/로그/테스트에서 문제가 확인됐는지 작성

## 제약
- 유지한 API, 건드리지 않은 파일, UX 고정 사항 작성

## 변경 내용
- 실제 변경 사항 작성

## 검증
- 실행한 명령과 결과 작성

## 보고서 반영 메모
- 이 PR이 프로젝트 품질 향상에서 어떤 의미인지 작성
```

## 5. 주차별 보고서 정리 방식

매주 아래 순서로 정리합니다.

1. GitHub에서 해당 주 PR 목록을 확인합니다.
2. Linear 코멘트에서 실제 문제와 캡처를 확인합니다.
3. `docs/engineering-history.md`의 표에 핵심 개선 흐름을 한 줄 추가합니다.
4. `docs/constraints-and-decisions.md`에 새 제약 또는 의사결정이 있으면 추가합니다.
5. Notion에는 보고서 문장 형태로 요약합니다.

Notion 항목은 단순 할 일 목록이 아니라 결과 보고서의 원천 자료입니다.
따라서 각 항목에는 최소한 `어려움`, `제약 사항`, `사용 기술`, `선택한 해결책`, `대안과 보류 이유`, `검증`, `결과`를 채웁니다.

## 6. 결과 보고서 섹션 초안

### 개발 과정의 문제와 개선

본 프로젝트는 Chrome Extension과 FastAPI backend를 연동해 실제 웹 페이지의 텍스트를 분석하고 마스킹하는 구조로 개발되었다.
개발 중 단순 키워드 차단 방식은 구현이 쉽지만 문맥 오탐이 크다는 문제가 확인되었고, 반대로 backend 모델 기반 분석은 정확도는 높지만 실시간 반영에 지연이 발생할 수 있다는 제약이 있었다.
따라서 최종 판정은 backend 모델의 `/analyze_batch` 응답으로 유지하되, extension에서는 visible container 우선 수집, exact span 마스킹, stale response guard, cache 분리를 적용하여 실제 사용 환경에서의 반응 속도와 안정성을 개선했다.
청마루의 차별점은 욕설 마스킹 자체가 아니라 문맥이 애매한 실제 플랫폼 환경에서 수집, 정규화, 모델 판단, evidence span, 사용자 정책, UI 반영을 하나의 중재 파이프라인으로 연결했다는 점이다.

### 제약 사항

Chrome Extension은 웹 페이지의 DOM 구조에 의존하기 때문에 Google 검색 결과, AI Overview, YouTube 댓글처럼 동적으로 변경되는 영역에서 후보 누락과 지연이 발생할 수 있었다.
또한 native input과 textarea는 일반 DOM text node처럼 부분 span을 삽입할 수 없어 검색창 마스킹은 별도의 fixed-token 방식으로 분리했다.
이러한 제약 때문에 전체 요소를 가리는 방식은 사용하지 않고, backend가 반환한 evidence span에만 마스킹을 적용하는 정책을 유지했다.
Android 접근성 수집 역시 앱마다 UI 트리 구조가 달라 같은 backend를 사용하더라도 수집 단계에서 누락과 중복이 발생할 수 있으므로, 플랫폼 수집 성공률도 평가 대상으로 관리한다.

### 품질 관리

오탐과 미탐은 실제 화면 캡처와 함께 regression case로 관리했다.
예를 들어 `카필 시발(Kapil Sibal)`처럼 고유명사 문맥은 safe로 유지하고, `시발 뭐하는 거야`처럼 직접 공격 문맥은 offensive로 판정되도록 테스트했다.
또한 GitHub PR과 Linear 코멘트를 통해 어떤 문제가 어떤 수정으로 이어졌는지 추적 가능하게 관리했다.
모델 비교는 단순 정확도 표가 아니라 keyword-only, ML-only, ML+normalization, ML+safe-context, full pipeline이 각각 어떤 실패 유형을 줄이거나 새로 만들었는지 설명하는 근거로 사용한다.

## 7. 지금부터 지켜야 할 운영 규칙

1. 새로운 문제를 발견하면 먼저 GitHub Issue Form에 “증거 중심”으로 남깁니다.
2. 코드 수정 PR은 반드시 “문제와 근거”, “제약”, “검증”을 포함합니다.
3. 보고서에 쓸 수 있는 개선은 `docs/engineering-history.md`에 한 줄로 정리합니다.
4. 단순히 “AI가 수정함”이라고 쓰지 않고 “어떤 제약 때문에 어떤 구조로 개선했는지”를 씁니다.
5. 입력창 `***` UX처럼 안정화된 부분은 별도 이슈 없이 다시 건드리지 않습니다.
6. Linear를 쓰는 경우에도 GitHub Issue 또는 PR 링크를 반드시 연결합니다.
7. Notion에는 GitHub 기준으로 검증된 항목만 주차별 보고서 형태로 옮깁니다.
8. 새 기능은 “문맥 판단, 불확실성 관리, 플랫폼 수집, evidence span 중재” 중 하나와 연결될 때만 보고서 핵심 개선으로 다룹니다.

## 8. 실제 작업 전후 체크

작업을 시작하기 전에는 현재 동작과 관련 제약을 먼저 확인합니다.
작업 중에는 코드 변경 이유를 PR 본문으로 바로 옮길 수 있게 기록합니다.
작업 후에는 자동 검증, 직접 API 호출, 실제 화면 확인 중 해당되는 근거를 남깁니다.

최소 기록 단위는 다음과 같습니다.

```md
- 문제:
- 제약:
- 개선:
- 검증:
- 남은 과제:
```

이 다섯 항목을 채울 수 없는 변경은 보고서에 쓸 수 있는 개선으로 보지 않습니다.

## 9. 어려운 문제를 보고서 근거로 바꾸는 방식

청마루에서 의미 있는 어려움은 “모델이 틀렸다” 또는 “마스킹이 늦다”처럼 증상만 남기면 보고서 근거가 약합니다.
아래처럼 원인을 분해해서 남겨야 실제 개선 과정으로 설명할 수 있습니다.

| 어려움 | 단순 기록 | 좋은 기록 |
| --- | --- | --- |
| 플랫폼 수집 한계 | YouTube 제목이 안 가려짐 | YouTube 제목이 일반 text node가 아니라 `contentDescription`에 합쳐져 있었고, 썸네일 내부 글자는 접근성 트리에 없어 OCR 없이는 후보 생성 불가 |
| 정확도/속도 충돌 | backend가 느림 | 전체 문맥을 보내면 정확도는 좋아지지만 지연이 커지고, 짧은 foreground 후보만 보내면 빠르지만 safe-context 판단이 약해짐 |
| UI 깨짐 | 박스가 이상함 | Android는 DOM span이 불가능해 좌표 overlay만 가능하고, composite card bounds를 그대로 쓰면 카드 전체가 가려짐 |
| 모델 한계 | 모델이 이상함 | boolean positive와 `evidence_spans`가 다를 수 있어, 화면 적용 가능 여부를 span 기준으로 별도 집계해야 함 |
| 설정 반영 문제 | 민감도가 이상함 | 비동기 backend 응답과 설정 변경이 race를 만들 수 있어 settings revision, cache key, stale response drop이 필요함 |

각 항목은 최소한 다음 네 가지로 정리합니다.

```md
- 관찰: 실제 화면/로그에서 무엇이 보였는가
- 원인 분해: 모델, 후보 수집, 좌표/렌더링, 네트워크, 설정 race 중 어디 문제인가
- 해결: 어떤 기술을 적용했고 왜 그 방식을 선택했는가
- 남은 제약: 아직 구조적으로 어려운 것은 무엇이고 다음 실험은 무엇인가
```

이 형식으로 남기면 “AI가 대신 구현했다”가 아니라 “실제 플랫폼 제약을 분석하고, 후보 수집/모델 판정/마스킹 UI/검증 체계를 단계적으로 개선했다”는 흐름을 만들 수 있습니다.

## 10. 이슈/PR/커밋 단위

청마루는 실험과 반복 수정이 많은 프로젝트이므로, 모든 시도를 main 기록에 그대로 남기면 오히려 개발 이력이 흐려집니다.
공식 이력은 아래 단위로만 남깁니다.

| 단위 | 남기는 기준 | 예시 |
| --- | --- | --- |
| GitHub Issue | 실제 문제가 관찰됐고 캡처, URL, 로그, backend 응답 중 하나 이상이 있을 때 | Google AI 개요 미탐, Android YouTube 제목 누락, sensitivity 0 stale mask |
| 작업 브랜치 커밋 | 구현 중간 저장 또는 실험이 필요할 때 | `fix/android-youtube-title-candidates` 안에서 여러 실험 커밋 허용 |
| PR | 문제 하나 또는 개선 목표 하나가 검증 가능한 형태로 묶였을 때 | Android YouTube title extraction + overlay bounds shrink |
| main merge commit | 리뷰/검증 후 보고서에 남길 수 있는 의미 단위일 때 | “Android YouTube 제목 후보 수집과 overlay 적용 안정화” |
| 문서 업데이트 | 제약, 의사결정, 검증 근거가 새로 생겼을 때 | OCR 필요성, 접근성 API 한계, backend authoritative 유지 |

PR 본문에는 반드시 `원인 분해`, `제약`, `검증`, `보고서 반영 메모`를 채웁니다.
이 항목을 채우기 어려운 변경은 아직 공식 개선으로 정리하기 이른 상태입니다.

커밋 정책은 다음과 같이 둡니다.

1. 작업 브랜치에서는 실험 커밋을 허용합니다.
2. main에는 기능 변화, 문제 해결, 검증 가능한 품질 개선처럼 의미가 분명한 PR만 merge합니다.
3. 단순 반복 수정, 임시 로그, 테스트 페이지 조정은 PR 안에서 정리하고 main 이력에는 남기지 않습니다.
4. 문서 변경은 코드 변경과 같은 PR에 포함하되, 보고서 근거가 실제로 생긴 경우에만 반영합니다.
5. 교수님 보고서에는 커밋 개수가 아니라 “문제 하나가 어떤 제약을 거쳐 어떻게 개선됐는지”를 중심으로 정리합니다.

## 11. Git 기준으로 상세 내용을 채우는 방법

작업 중에는 기록을 짧게 남기고, 보고서 작성 시점에는 GitHub/Git 이력으로 상세 내용을 채웁니다.

```bash
git log --oneline --max-count=20
git show --stat <commit>
gh pr view <number>
gh issue view <number>
```

각 PR 또는 커밋에서 아래 항목만 뽑으면 보고서 문단으로 바꿀 수 있습니다.

```md
- 문제: PR의 문제와 근거
- 제약: PR의 제약 코드와 유지한 계약
- 해결: 변경 내용과 선택한 방식
- 검증: 자동 테스트, 직접 호출, 실제 화면 확인
- 결과: 해결/부분 해결/보류와 남은 과제
```

이 방식의 목적은 개발 중 문서 작성 부담을 줄이되, 나중에 “어떤 문제를 어떤 근거로 개선했는지”를 재구성할 수 있게 만드는 것입니다.

## 12. Evidence ledger 자동화

반복 실험이 많은 Android/extension/backend 작업은 커밋만으로 의미를 파악하기 어렵습니다.
따라서 의미 있는 실패, 실험, 개선, 회귀, 검증, 의사결정은 `scripts/chungmaru_evidence.py`로 단일 JSONL ledger에 기록합니다.

공식 ledger는 다음 두 파일입니다.

| 파일 | 역할 |
| --- | --- |
| `docs/evidence/chungmaru-progress-log.jsonl` | append-only 원본 기록 |
| `docs/evidence/chungmaru-progress-index.md` | 발표/보고서용 요약 인덱스 |

사용 기준은 다음과 같습니다.

1. raw 실행 JSON을 계속 커밋하지 않습니다. 필요한 경우 compact fixture 또는 curated artifact만 남깁니다.
2. branch의 작은 실험 커밋은 `backfill-git`으로 추적하되, 최종 보고서에 쓰려면 수동 failure/improvement 항목으로 증거를 보강합니다.
3. 사용자가 캡처로 보여준 새로운 이상 현상은 먼저 `failure`로 등록하고, 다음 커밋이 그 실패를 줄였는지 별도 `improvement` 또는 `verification`으로 남깁니다.
4. Android 실시간 마스킹은 기본적으로 `BBA-79`, `C4`, `C5`, `C7`을 함께 확인합니다.
5. PR 본문과 Linear 코멘트는 `docs/evidence/chungmaru-progress-index.md`의 항목을 기준으로 작성합니다.
6. 개발 세션과 등록 세션을 분리합니다. `source_session_id`는 실제 오류/개발/검증이 나온 세션이고, `registrar_session_id`는 이 내용을 ledger/Notion에 정리한 세션 또는 자동화입니다.
7. `evidence_quality=commit-only`는 추적용입니다. 최종 보고서에 쓰려면 최소 `log-backed`, 화면 증거가 있으면 `screenshot-backed`, 검증 중심이면 `test-backed`, 표/fixture/PPT 자산이면 `artifact-backed`, 바로 삽입 가능한 항목이면 `report-ready`로 올립니다.
8. 개발 세션이 계속 이어질 때는 worktree를 실험 격리 장치로 쓰고, official checkout은 ledger/Notion/PR 정리 기준으로 둡니다.
9. worktree에서 나온 증거는 `worktree_path`, `worktree_role`, `source_branch`, `integration_branch`를 함께 기록합니다.
10. Notion에는 모든 ledger row를 옮기지 않습니다. 심사 대응에 필요한 대표 문제, blocker, report-ready 후보만 기존 카드 중심으로 갱신합니다.
11. 매번 정리할 때는 모델, 파서/추출기, 통합 흐름 중 3-5개 대표 문제만 뽑아 board 상태를 갱신합니다.

기본 명령은 아래와 같습니다.

```bash
python3 scripts/chungmaru_evidence.py backfill-git \
  --revision-range origin/main..HEAD \
  --linear-issue BBA-79 \
  --github-pr https://github.com/BbangYi/ChungMaru/pull/33 \
  --source-session-id 019e1b02-4966-7d50-ac1b-ec191478dcd6 \
  --registrar-session-id 019e3546-cfae-7450-880d-006c2f1102d5 \
  --worktree-role experiment \
  --source-branch codex/android-mask-latency-diagnostics \
  --integration-branch main \
  --write

python3 scripts/chungmaru_evidence.py audit-worktrees
python3 scripts/chungmaru_evidence.py validate
```

## 13. Worktree 운영 원칙

worktree는 기록 저장소가 아니라 작업 격리 장치입니다.
Git history는 최종 PR/merge에서 얕게 정리하고, 실패와 실험의 자세한 맥락은 evidence ledger에 남깁니다.

| 상황 | 권장 위치 | 이유 |
| --- | --- | --- |
| Android 실기기, overlay, OCR, scroll 실험 | 별도 worktree | dirty 상태와 장시간 실험 로그가 official checkout을 막지 않게 함 |
| backend 재학습, 평가 비교, 모델 파일 실험 | 별도 worktree | 모델 산출물과 평가 로그가 main 정리를 방해하지 않게 함 |
| Chrome/extension E2E 검증 | 별도 worktree 또는 verification worktree | 브라우저/서버 상태와 extension 빌드 상태를 분리함 |
| docs/evidence/Notion 정리 | official checkout | 공식 보고 자료와 ledger 원본을 한 곳에 유지함 |

권장 경로는 `/Users/giminu0930/Documents/000 Project/20_도구실험/worktrees/` 아래입니다.
같은 브랜치는 두 worktree에서 동시에 checkout하지 않습니다.
worktree 실험이 끝나면 PR에 올릴 의미 단위만 남기고, ledger에는 실패/개선/검증/결정 단위로 기록합니다.

## 14. 심사 대응 우선순위

2026년 5월 14일 To-do-list는 최종 보고서/발표 자료에 직접 들어갈 증거 목록입니다.
따라서 자동화나 개발 운영보다 아래 세 축을 먼저 채웁니다.

| 축 | 우선 증거 | 평가 연결 |
| --- | --- | --- |
| 탐지 모델 | 재학습 전/후 성능, Accuracy/Precision/Recall/F1/FP/FN, category별 성능, clean-topic bias, FP/FN 대표 사례, score bucket, span detection | 기술성 |
| 파서/추출기 | raw vs cleaned JSONL, UI 제거 규칙, 댓글 없음/다른 화면 감지, 중복 제거, 플랫폼별 오류, Android boundsInScreen | 기술성 |
| 통합 흐름 | Chrome/Android E2E, cleaned 댓글 -> 정규화 -> 모델 판단 -> evidence span -> 최종 마스킹, demo 가능/어려운 기능 구분 | 완성도 |

Notion board는 이 세 축의 상태판입니다.
전체 이력은 repo ledger에 남기고, Notion에서는 발표/보고서에 들어갈 대표 항목과 아직 막힌 항목을 유지합니다.
