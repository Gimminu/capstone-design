...
def render_index(entries: list[dict[str, Any]], index_path: Path, log_path: Path) -> None:
    ...
    for label, counter in (("Event type", by_type), ("Lane", by_lane), ("Outcome", by_outcome)):
...
def cmd_add(args: argparse.Namespace) -> int:
    ...
    entry = {
        ...
        "timestamp": utc_now(),
        "source": "manual",
        "session_id": args.session_id or os.environ.get("CODEX_SESSION_ID"),
        "branch": current_branch(),
...
def cmd_backfill_git(args: argparse.Namespace) -> int:
    ...
    new_entries = [
        build_commit_entry(row, args.linear_issue, args.github_pr, args.session_id)
        for row in rows
