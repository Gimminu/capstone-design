...
PROFANITY_PATTERNS = [
    # ── 비속어 (profanity) ──────────────────────────────
    # 씨발 계열
    (r"씨[이\s]*발|시[이\s]*발|씨[이\s]*팔|시[이\s]*팔|씨[이\s]*벌|ㅅㅂ|ㅆㅂ|씨[0-9]*발|시[0-9]*발|싸[이]?발|쒸[이]?발|씌발|c발|씨bal|니미씨|니미시|시불|시블|씨부[랄럴렁]|시부[랄럴렁]|s{1,2}[\W_]*i[\W_]*b[\W_]*a[\W_]*l|t[\W_]*l[\W_]*q[\W_]*k[\W_]*f", "씨발", "profanity"),
    # 개새끼 계열
    (r"개[새세][끼키]|개[새세]끼|개[새세]기|개색[끼키]|개[쌔쎄][끼키]|개[0-9]*새끼|ㄱㅅㄲ", "개새끼", "profanity"),
    # 병신 계열
    (r"병[\.]*신|byung[s]?in|ㅂㅅ|병[0-9]*신|빙신|벼ㅇ신|병딱|병1신|q[\W_]*u[\W_]*d[\W_]*t[\W_]*l[\W_]*s", "병신", "profanity"),
    # 지랄 계열
    (r"지[이\s]*랄|ㅈㄹ|지[0-9]*랄", "지랄", "profanity"),
    # 좆/좃 계열
    (r"좆|좇|조[까깟]|졸라|존나|ㅈㄴ|존[0-9]*나|좃|좋까|존[나]?맛|좆같", "좆", "profanity"),
    # 씹 계열
    (r"씹|십[새세][끼키]|씹[새세][끼키]|씹할|ㅆ[0-9]*ㅂ|씹덕", "씹", "profanity"),
    # 꺼져/닥쳐 계열
    (r"꺼[져저]|닥[쳐쳐]|닥치|닥[0-9]*쳐|꺼[0-9]*져", "꺼져", "profanity"),
    # 미친 계열
    (r"미친[놈년]|미친[새]끼|미[0-9]*친|ㅁㅊ|미놈|미년", "미친", "profanity"),
    # 새끼 단독
    ...
    # 영문 욕설
    (r"\bmother[\W_]*fucker\b|\bfuck(?:ing|er|ed)?\b", "fuck", "profanity"),
    (r"\bass[\W_]*hole\b|\bbitch(?:es)?\b|\bshit(?:ty|head|s)?\b|\bbastard(?:s)?\b", "영문비속어", "profanity"),
    # 개 (접두 강조) + 부정 표현
